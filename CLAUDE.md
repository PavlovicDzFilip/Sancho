...
- Transcription: three `ITranscriptionService` implementations selected by the `transcription` config key (`openai` is the default): `RealtimeTranscriptionService` (OpenAI Realtime WebSocket), `LocalTranscriptionService` (on-device sherpa-onnx offline zipformer en int8 + silero VAD; utterances arrive as `Completed` events at utterance end, no live deltas; model files downloaded to `~/.sancho/models/` on first use; `LocalSttModels` handles provisioning), and `RecordingTranscriptionService` (writes the PCM stream to a WAV in `~/.sancho/recordings/`, pure managed). Engine choice: `docs/feature/local-stt/ADR-0002-offline-zipformer-vad.md` (supersedes ADR-0001).
