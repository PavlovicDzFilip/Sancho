...
- Transcription: `LocalTranscriptionService` (the only backend — on-device sherpa-onnx offline whisper .en int8 + silero VAD, size selectable via `--model`/`model` config key (tiny/base/small/medium, default small); utterances arrive as `Completed` events at utterance end, no live deltas; model files downloaded to `~/.sancho/models/<size>/` at startup (right after the size is resolved), before capture or transcription begin; `WhisperModels` holds the size→repo mapping, `LocalSttModels` handles provisioning). The OpenAI and record backends were removed — sancho is fully offline (ADR-0004). Engine choice: `docs/feature/local-stt/ADR-0003-offline-whisper-vad.md` (supersedes ADR-0002).
- Agent integration: `Services\Agents\` — `AgentService` abstraction over the selected CLI, all backends emit the shared `AgentEvent` stream; `ClaudeCodeAgentService` drives any Claude-Code-protocol CLI (subprocess via `<agent> --print --input-format stream-json --output-format stream-json`); `CursorAgentService` runs one-shot `cursor-agent -p --output-format stream-json` per turn, chaining `--resume <session_id>`; `HermesAgentService` runs one-shot `hermes -z` per turn, discovering the session id for `--pass-session-id`; `CodexAgentService` runs one-shot `codex exec --json` per turn, chaining `codex exec resume <session_id>`; the `agent` config key (or `--agent` flag) selects the backend (claude, cursor, hermes, codex). The agent factory in Program.cs verifies the executable exists before the orchestrator starts; session pickers read each agent's own store (`SessionJson` handles the claude/cursor/codex line formats; hermes parsing is best-effort against its CLI output).
- Pipeline: mic → Channel<byte[]> → `LocalTranscriptionService` → Channel<string> → Claude CLI

## Configuration
- File: `~/.sancho/config.json` (`SANCHO_CONFIG_DIR` env var overrides the directory). Keys: `agent` (default `claude`), `model` (default `small`; `tiny`/`base`/`medium` also supported).
...
- `sancho --agent <name>` — one-off agent override (default `claude`; `cursor`, `hermes` and `codex` supported); persisted via `sancho config set agent <name>`
- `sancho --model <size>` — one-off whisper size override (default `small`; `tiny`, `base` and `medium` supported); persisted via `sancho config set model <size>`
